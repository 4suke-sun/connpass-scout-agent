...
function buildSearchPrompt(keywords: string): string {
  ...
  return [
    ...
    "- 各キーワード上位3件まで",
    "",
    "検索結果のフィールド: title, catchCopy(概要), description(イベント説明文・要約の主な材料), " +
      "url, startedAt, place(開催場所/オンラインの場合は null), accepted(参加確定数), " +
      "limit(定員/nullは無制限), waiting(キャンセル待ち数)",
    "",
    ...
    "　📍 place",
    "　_catchCopy と description を両方読んだうえでイベント内容を30文字以内で要約_",
    "",
    "- limit が null の場合は「定員なし」と表記",
    "- waiting が 0 の場合は「(待ちN名)」を省略",
    "- place が null の場合は📍行を省略(オンライン開催の可能性が高い)",
